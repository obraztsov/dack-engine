...
//! (consciousness substrate) is rented from OpenClaude over localhost gRPC
//! ([`runtime`]); the harness is the dumb, deterministic subconscious around it.
//!
...
//! 2. **Filters on cognition: no. Boundaries on irreversibility: yes.** The wall
//!    ([`runtime::settle`]) sits only on irreversible doorways; cognition is sovereign.
...
//! - [`runtime`]  — the OpenClaude gRPC seam + the `action_required` wall + `allow_settle`
//! - [`identity`] / [`repo`] / [`memory`] — the swappable seams (Gitlawb v1; corp later)
